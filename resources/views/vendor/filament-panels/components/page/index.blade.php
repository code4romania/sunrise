    ...
    <div class="fi-page-header-main-ctn">
        ...
        @else
            @php
                ...
                $leadingActions = array_values(array_filter($headerActions, fn ($action) => method_exists($action, 'getName') && $action->getName() === 'back_action'));
                $headerActions = array_values(array_filter($headerActions, fn ($action) => ! method_exists($action, 'getName') || $action->getName() !== 'back_action'));
                $headerActionsAlignment = $this->getHeaderActionsAlignment();
                $breadcrumbs = filament()->hasBreadcrumbs() ? $this->getBreadcrumbs() : [];
                $subheading = $this->getSubheading();
            @endphp

            @if (filled($leadingActions) || filled($headerActions) || $breadcrumbs || filled($heading) || filled($subheading))
                <x-filament-panels::header
                    :actions="$headerActions"
                    :actions-alignment="$headerActionsAlignment"
                    :breadcrumbs="$breadcrumbs"
                    :heading="$heading"
                    :leading-actions="$leadingActions"
