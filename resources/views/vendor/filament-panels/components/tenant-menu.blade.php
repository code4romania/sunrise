@php
    use App\Models\Scopes\BelongsToCurrentTenant;use App\Models\UserStatus;$currentTenant = filament()->getTenant();
    ...
    $canSwitchTenants = count($tenants = array_filter(
        ...
        fn (\Illuminate\Database\Eloquent\Model $tenant): bool =>
            ! $tenant->is($currentTenant) &&
            ! $tenant->load('institution')
                ->institution
                ->isInactivated() &&
            ! UserStatus::query()->where('user_id', auth()->user()->id)
